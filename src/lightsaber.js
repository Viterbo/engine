...
LightSaber.prototype = {
    ...
    extend_spec: function (spec) {
        ...
        if (spec.instance_name && this._settings.spec.scene.sections) {
            var sec = this._settings.spec.scene.sections[this._section];            
            if (sec && sec[spec.instance_name]) {
                var _section_spec = sec[spec.instance_name];
                _section_spec = this.extend_spec(_section_spec);
